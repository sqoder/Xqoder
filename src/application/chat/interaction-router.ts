...
const ENGINEERING_INTENT_TRIGGER = /(?:src\/|test\/|dist\/|\.ts\b|\.tsx\b|\.js\b|\.jsx\b|\.json\b|\.md\b|bug|error|exception|typeerror|build|test|lint|coverage|cli|tui|mvp|stack trace|fix|implement|refactor|debug|inspect|read|search|run|execute|delete|remove|edit|modify|write|regression|continue current task|continue the task|add (?:a )?feature|new feature|按.*文档|根据.*文档|继续当前任务|继续这个任务|继续任务|继续推进|接着推进|继续完成|继续修|继续改|修复|改代码|修改代码|实现|接入|新增功能|新增一个功能|新增特性|删除|清理|检查|搜索|读取|运行|构建|测试|调试|重构|报错|异常|需求|代码|文件|函数|接口|类型|测试用例|命令|脚本|继续处理)/i;
