/**
 * Session 落盘脱敏（Day 13）
 * 仅用于写入 DB 前的副本，不修改内存中的 session。
 * 规则见 docs/session-sanitization-rules.md
 */
...
/** 第一版：对单段文本做脱敏，用于 last_user_message 与 message 内 content/thinking 等 */
export function sanitizeForPersistence(text: string): string {
    ...
    // key=value / key: value 敏感键
    out = out.replace(
        /\b([Aa]pi[_-]?[Kk]ey|[Ss]ecret|[Tt]oken|[Pp]assword)\s*[:=]\s*['"]?[^\s'"]*['"]?/gi,
        (_, key) => `${key}=${REDACT}`,
    );

    // .env 风格行：KEY=value，键在敏感集合内则值替换
...
/** 深拷贝并脱敏单条 LLMMessage，用于写入 session_messages.message_json */
