...
export interface SaveSessionOptions {
    /** 是否持久化 last_user_message（Day 14）。默认 true；设为 false 时高敏内容不落盘。 */
    ...
    saveSession(input: SaveSessionInput): PersistedSessionSummary {
        ...
        try {
            ...
            snapshot.messages.forEach((message, index) => {
                const sanitized = sanitizeMessageForPersistence(message as Parameters<typeof sanitizeMessageForPersistence>[0]);
...
function ensureSessionSchema(db: DatabaseSync): void {
    db.exec(`
