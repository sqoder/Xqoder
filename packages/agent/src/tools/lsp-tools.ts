...
import type { LSPServerConfig, ToolDefinition, ToolResult } from '@xqoder/shared';
import {
    type CompletionMatch as LspCompletionMatch,
...
function getOrCreateTsContext(projectRoot: string): TypeScriptWorkspaceContext {
    const key = path.resolve(projectRoot);
    const now = Date.now();
    const cached = tsContextCache.get(key);
    if (cached) {
        if (now - cached.at < TS_CONTEXT_CACHE_TTL_MS) return cached.ctx;
    ...
    if (tsContextCache.size >= TS_CONTEXT_CACHE_MAX) {
        ...
        if (oldest) tsContextCache.delete(oldest[0]);
    }
...
export class LspWorkspaceSymbolsTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        ...
        description: '按符号名搜索当前项目中的定义，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'query', type: 'string', description: '要搜索的符号名片段', required: true },
            { name: 'limit', type: 'number', description: '最多返回多少条结果，默认 20', required: false },
        ],
    };
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP 符号搜索失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
}
...
export class LspFileDiagnosticsTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        ...
        description: '获取文件诊断信息，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP 诊断失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }

    private async getDiagnostics(filePath: string, projectRoot: string): Promise<LspDiagnosticMatch[]> {
...
export class LspDefinitionTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        name: 'lsp_definition',
        description: '根据文件位置跳转到符号定义，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'path', type: 'string', description: '目标文件路径', required: true },
            { name: 'line', type: 'number', description: '1-based 行号', required: true },
            { name: 'character', type: 'number', description: '1-based 列号', required: true },
        ],
    };
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP 定义查询失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
...
export class LspReferencesTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        name: 'lsp_references',
        description: '根据文件位置查找符号引用，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'path', type: 'string', description: '目标文件路径', required: true },
            { name: 'line', type: 'number', description: '1-based 行号', required: true },
            { name: 'character', type: 'number', description: '1-based 列号', required: true },
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP 引用查询失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
...
export class LspHoverTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        name: 'lsp_hover',
        description: '读取文件位置的 hover 信息，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'path', type: 'string', description: '目标文件路径', required: true },
            { name: 'line', type: 'number', description: '1-based 行号', required: true },
            { name: 'character', type: 'number', description: '1-based 列号', required: true },
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP hover 查询失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
...
export class LspCompletionTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        name: 'lsp_completion',
        description: '读取文件位置的补全候选，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'path', type: 'string', description: '目标文件路径', required: true },
            { name: 'line', type: 'number', description: '1-based 行号', required: true },
            { name: 'character', type: 'number', description: '1-based 列号', required: true },
            { name: 'limit', type: 'number', description: '最多返回多少条结果，默认 20', required: false },
            { name: 'resolveDetails', type: 'boolean', description: '是否对前几个候选补全执行 completionItem/resolve', required: false },
            { name: 'resolveLimit', type: 'number', description: '最多解析多少个补全项，默认 5', required: false },
        ],
    };
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP completion 查询失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
...
export class LspRenameSymbolTool implements ITool {
    ...
    readonly definition: ToolDefinition = {
        ...
        description: '按文件位置重命名符号，支持内置 TypeScript/JavaScript 和已配置的外部 LSP server。',
        parameters: [
            { name: 'path', type: 'string', description: '目标文件路径', required: true },
            { name: 'line', type: 'number', description: '1-based 行号', required: true },
            { name: 'character', type: 'number', description: '1-based 列号', required: true },
            { name: 'newName', type: 'string', description: '新的符号名', required: true },
        ],
    };
    ...
    async buildApprovalRequest(
        ...
        return {
            ...
            summary: `重命名 ${renamePlan.filePaths.length} 个文件中的符号为 ${renamePlan.newName}`,
            reason: '该操作会批量修改代码引用，执行前需要确认 diff。',
    ...
    async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
        ...
        try {
            const renamePlan = await this.prepareRenamePlan(args, context);
            if (!renamePlan) {
                throw new Error('未生成任何 rename edit');
            }
            ...
            const outputLines = [
                `已重命名符号为 ${renamePlan.newName}`,
                `影响文件: ${renamePlan.filePaths.join(', ')}`,
                `编辑数: ${renamePlan.totalEdits}`,
            ];
            if (rollbackPoint) {
            ...
            return {
                ...
                success: false,
                output: '',
                error: `LSP rename 失败: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
...
class TypeScriptWorkspaceContext {
    ...
    renameSymbol(
        ...
        if (!renameInfo.canRename) {
            throw new Error(renameInfo.localizedErrorMessage || '当前位置不可重命名');
        }
    ...
    private ensureTrackedFile(filePath: string): void {
        ...
        if (!isSupportedSourceFile(resolvedFilePath)) {
            throw new Error(`当前仅支持 TS/JS 文件: ${resolvedFilePath}`);
        }
...
function formatSymbolMatch(match: LspSymbolMatch): string {
    return [
...
function formatCompletionMatch(match: LspCompletionMatch): string {
    return [
...
function groupTextEditsByFile(edits: LspTextEditMatch[]): Map<string, LspTextEditMatch[]> {
...
function applyTextEditsToContent(content: string, edits: LspTextEditMatch[]): string {
    ...
    for (const edit of normalized) {
        if (edit.startOffset > edit.endOffset) {
            throw new Error(`无效的 text edit 范围: ${edit.filePath}`);
        }
        if (edit.endOffset > lastStart) {
            throw new Error(`检测到重叠的 text edits: ${edit.filePath}`);
        }
...
function clampResolveLimit(value: unknown): number {
