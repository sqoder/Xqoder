...
function renderRootQualityPage({ benchmarkReports, evalReports }) {
  const lines = [
    '# Quality Report',
    '',
    `- Generated: ${new Date().toISOString()}`,
...
function renderOpenCodeComparisonPage({ benchmarkReports, evalReports }) {
  ...
  const lines = [
    ...
    `| 质量证据 | 真实项目页当前 \`${formatCountCell(realProjectTargets)}\` 个目标、\`${formatCountCell(realProjectWorkflowRuns)}\` 次 workflow run、\`${realProjectAcceptance}\`；Benchmark/Eval 页面均已公开 | 对外叙事不靠形容词，靠可重复运行的数据页面 |`,
    '',
    '## Public Metrics',
    '',
